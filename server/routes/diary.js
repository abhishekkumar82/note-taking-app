...
router.delete("/reset-encryption", async (req, res) => {
  try {
    ...
    console.log(`[diary/reset-encryption] deleted ${result.deletedCount} entries for user ${req.user._id}`);
    res.json({ message: `Deleted ${result.deletedCount} encrypted diary entries.`, deleted: result.deletedCount });
  } catch (err) {
    console.error("[diary/reset-encryption]", err.message);
