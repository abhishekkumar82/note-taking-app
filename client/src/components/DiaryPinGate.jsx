...
// ─────────────────────────────────────────────────────────────────────────────
// Secure diary PIN gate — now also derives the AES-GCM CryptoKey from the PIN
// and passes it up to DiaryPage so every entry is encrypted/decrypted in the
// browser. The backend only ever stores ciphertext.
//
// Changes vs old version:
//  • deriveKey() called on successful PIN entry → onUnlocked(cryptoKey)
//  • Forgot-PIN flow clears the localStorage salt so old ciphertext becomes
//    permanently unreadable (entries are deleted from DB on PIN reset — see
//    diary.js route below for the /api/diary/reset-encryption endpoint).
// ─────────────────────────────────────────────────────────────────────────────

import React, { useState, useRef } from "react";
import api from "../utils/axiosInstance";
import { deriveKey, getOrCreateSalt } from "../utils/diaryEncryption";

// ── Simple djb2 hash — PIN verification stored in localStorage as hash only ──
const hashPin = (pin) => {
  let h = 5381;
...
const OtpInput = ({ value, onChange }) => {
  ...
  const kd = (i, e) => {
    if (e.key === "Backspace") {
      e.preventDefault();
      const n = [...digits];
      if (n[i]) { n[i] = ""; } else if (i > 0) { n[i - 1] = ""; refs.current[i - 1]?.focus(); }
      onChange(n.join(""));
...
// ── Main DiaryPinGate ─────────────────────────────────────────────────────────
// onUnlocked(cryptoKey) — passes the derived AES-GCM key up to DiaryPage
const DiaryPinGate = ({ userId, onUnlocked }) => {
  ...
  // ── Derive key + call onUnlocked ──────────────────────────────────────────
  const unlockWithPin = async (resolvedPin) => {
    setLoading(true);
    try {
      const salt = getOrCreateSalt(userId);
      const key  = await deriveKey(resolvedPin, salt);
      onUnlocked(key);
    } catch {
      setErr("Failed to derive encryption key. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const doEnter = async () => {
    ...
    if (hashPin(pin) !== stored) {
      setErr("Wrong PIN. Try again."); setPin(""); trigShake(); return;
    }
    await unlockWithPin(pin);
  };
  ...
  // ── Forgot PIN — Step 2: verify OTP, wipe encrypted entries, set new PIN ──
  // We MUST delete old encrypted entries from the DB here because they were
  // encrypted with the old PIN-derived key — they cannot be recovered.
  const verifyPinReset = async () => {
    ...
    try {
      ...
      // Delete all encrypted diary entries — they're unrecoverable without the old key
      // The backend /api/diary/reset-encryption endpoint handles this safely
      await api.delete("/api/diary/reset-encryption");

      // Clear old PIN hash + salt so a fresh salt is created for the new PIN
      localStorage.removeItem(pinKey);
      localStorage.removeItem(`wu_diary_salt_${userId}`);

      setMsg("Code verified! Old encrypted entries deleted. Please set your new Diary PIN.");
  ...
  const S = {
    ...
    btn:    { width: "100%", padding: "12px", borderRadius: 11, border: "none", background: loading ? "#c7d2fe" : "linear-gradient(135deg,#6366f1,#8b5cf6)", color: "#fff", fontSize: 15, fontWeight: 700, cursor: loading ? "default" : "pointer", fontFamily: "inherit", transition: "opacity .15s,transform .15s", marginBottom: 10 },
    ...
    warn:   { background: "#fff7ed", border: "1px solid #fed7aa", borderRadius: 8, padding: "9px 12px", fontSize: 12.5, color: "#9a3412", marginBottom: 14, lineHeight: 1.5 },
  };
      ...
      <div style={{ ...S.card, transform: shake ? "translateX(-4px)" : "none" }}>
        ...
        {screen === "enter" && (
          <>
            ...
            <div style={S.sub}>Enter your PIN to decrypt and open your diary.</div>
            ...
            <button style={S.btn} onClick={doEnter} disabled={loading}>
        ...
        {screen === "set" && (
          <>
            ...
            <div style={S.sub}>Your PIN encrypts all diary entries in the browser — even we can't read them.</div>
            <div style={S.warn}>
              🔐 <strong>Remember your PIN.</strong> If you forget it, encrypted entries cannot be recovered (only deleted).
            </div>
            {err && <div style={S.err}>{err}</div>}
            {msg && <div style={S.msg}>{msg}</div>}
            <div style={S.inpWrap}>
        ...
        {screen === "confirm" && (
          <>
            ...
            <button style={S.btn} onClick={doConfirm} disabled={loading}>
              {loading ? "Deriving key…" : "Confirm & Open Diary →"}
            </button>
          </>
        )}
        ...
        {screen === "forgotStep1" && (
          <>
            ...
            <div style={S.warn}>
              ⚠️ <strong>All encrypted diary entries will be permanently deleted.</strong> They cannot be recovered without the old PIN. Only reset if you have truly forgotten it.
            </div>
            {err && <div style={S.err}>{err}</div>}
            {msg && <div style={S.msg}>{msg}</div>}
        ...
        {screen === "forgotStep2" && (
          <>
            ...
            <div style={S.sub}>
              Enter the 6-digit code sent to your <strong>{sentVia === "phone" ? "phone" : "email"}</strong>. Valid 10 minutes.
            </div>
            {err && <div style={S.err}>{err}</div>}
            {msg && <div style={S.msg}>{msg}</div>}
            <OtpInput value={resetOtp} onChange={setResetOtp} />
            <button style={{ ...S.btn, opacity: loading || resetOtp.length < 6 ? 0.6 : 1 }}
              disabled={loading || resetOtp.length < 6} onClick={verifyPinReset}>
