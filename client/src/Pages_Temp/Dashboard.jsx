...
import { getNotesForDisplay, pullFromServer } from "../utils/syncEngine";
import { createNoteOffline, updateNoteOffline, deleteNoteOffline, trySyncNow } from "../utils/noteSync";
...
const Dashboard = () => {
  ...
  const [commandPaletteSignal, setCommandPaletteSignal] = useState(0);
...
const fetchNotes = useCallback(async () => {
  try {
    // Always pull fresh data from the server first if online (keeps
    // IndexedDB current), then read from IndexedDB for actual display —
    // this means the UI logic is identical whether online or offline.
    if (navigator.onLine) {
      await pullFromServer();
    }
    const localNotes = await getNotesForDisplay();
    const activeNotes = localNotes.filter(n => !n.isDiary && !n.isArchived);
    setNotes(activeNotes);

    // userName still comes from the live API response when available;
    // fall back to whatever's cached if offline on first load.
    if (navigator.onLine) {
      try {
    ...
    const fl = [...new Set(activeNotes.map(n => typeof n.folder === "string" ? n.folder : "General"))]
...
const handleTemplateSelect = (template) => {
  ...
  const handleSemanticResults = (results, term) => {
  // Semantic results come pre-shaped close to Note objects but without
  // every field NoteCard might expect (isDeleted, isArchived, etc.) —
  // backfill safe defaults so NoteCard renders without errors.
...
const handleUpdateNote = async (id, title, body, color, reminder, repeat, isPinned) => {
  try {
    const updated = await updateNoteOffline(id, { title, body, color, reminder, repeat, isPinned });
    setNotes(prev => prev.map(n => n._id === id ? updated : n));
    setArchivedNotes(prev => prev.map(n => n._id === id ? updated : n));
  } catch (e) { console.error(e); }
};
...
<CommandPalette
  notes={notes}
  openSignal={commandPaletteSignal}
  onNavigate={handleSetActiveSection}
  onOpenNote={(note) => setEditingNote(note)}
  onNewNote={() => { setActiveSection("notes"); setAddNoteOpen(true); }}
  onOpenTemplate={() => { setActiveSection("notes"); setShowTemplatePicker(true); }}
  />
      <Header
        onSearch={handleSearch}
        onSemanticResults={handleSemanticResults}
        onOpenCommandPalette={() => setCommandPaletteSignal(v => v + 1)}
...
{activeSection === "notes" && (
  <div className="add-note-area">
    {!addNoteOpen ? (
      <button className="add-note-trigger" onClick={() => setShowTemplatePicker(true)}>
        <Plus size={16} /> Take a note…
      </button>
    ) : (
      <AddNote
        onNoteAdded={(note) => { handleNoteAdded(note); setSelectedTemplate(null); }}
        folders={folders}
        showToastMsg={showToastMsg}
